...
/* Icon circle — very soft */
.cldd-mi-icon {
  width:48px; height:48px; border-radius:12px;
  display:flex; align-items:center; justify-content:center;
  font-size:20px; margin:0 auto 14px;
}
.cldd-mi-icon.info    { background:#f8fafc; }
.cldd-mi-icon.success { background:#f0fdf4; }
.cldd-mi-icon.error   { background:#fff5f5; }
.cldd-mi-icon.warning { background:#fffbeb; }
...
function modal({ title='', message='', type='info', okText='OK', cancelText=null }) {
  return new Promise(res => {
    ...
    m.innerHTML = `
      <div class="cldd-mi-icon ${type}">${ICONS[type]||'·'}</div>
